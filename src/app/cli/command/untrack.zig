...
pub fn run(allocator: std.mem.Allocator, args: parser_types.UntrackArgs) !command_types.CommandResult {
    ...
    if (args.missing) {
        var outcome = try track_ops.untrackMissing(allocator, omohi.dir);
        defer track_ops.freeUntrackMissingOutcome(allocator, &outcome);

        const output = try presenter.untrackMissingResult(allocator, &outcome);
